...
const PIXEL_TO_UNIT_RATIO: f32 = 1.0;

// A collider generated for the image specified by it's path
#[derive(Resource, Default)]
pub struct ImageColliderMap(pub HashMap<String, Collider>);
...
impl Contour {
    ...
    pub fn to_simplified_line_strings(&self, max_edge_len: u32) -> Vec<LineString<f32>> {
        ...
        while !todo.is_empty() {
            ...
            for i in 0..contained_by.len() {
                ...
                if contained_by[i].len() == 0 {
                    res.push(geo::Polygon::new(
                        line_strings[exterior_index].clone(),
                        vec![],
                    ));
            ...
            for i in 0..line_strings.len() {
                ...
                if set.len() == 1 && set.contains(&exterior_index) {
                    res.last_mut()
                        .unwrap()
                        .interiors_push(line_strings[i].clone());
                    done.push(i);
                }
            }
